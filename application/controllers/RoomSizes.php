    ...
    function addedNewRoomSize()
{
    ...
    else
    {
        ...
        else
        {
            ...
            $foto = $this->roomSizes_model->uploadFoto($_FILES['picture_product']);

            if (isset($foto['error'])) {
                $dataError = array('error' => $foto['error']);
                $this->session->set_flashdata('error', $dataError['error']);
                redirect('addNewRoomSize');
            }

            $roomSizeInfo = array('sizeTitle'=>$roomSizeTitle, 'sizeDescription'=>$roomSizeDescription,
                'createdBy'=>$this->vendorId, 'createdDtm'=>date('Y-m-d H:i:sa'), 'picture_product' => $foto['file_name']);
    ...
    function editOldRoomSize($roomSizeId = NULL)
{
    ...
    else
    {
        ...
        // Check if the room size information exists before rendering the view
        if (empty($data['roomSizeInfo'])) {
            redirect('roomSizeListing');
        }
